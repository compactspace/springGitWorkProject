...
.status-rejected {
	background-color: #f44336; /* 빨강 → 거절 */
	color: #fff;
}

.status-insufficient {
	background-color: #FF9800; /* 주황 → 보완 요청 */
	color: #fff;
}

.status-resubmit {
	background-color: #2196F3; /* 파랑 → 재제출 요청 */
	color: #fff;
}
/* 공통 CSS종료  */
</style>
								...
								<td><c:choose>
										...
										<c:when test="${doc.status == 'Rejected'}">
											<span class="status-badge status-rejected"
												title="${doc.review_comment}">거절</span>
										</c:when>
										<c:when test="${doc.status == 'Insufficient'}">
											<span class="status-badge status-insufficient"
												title="서류 보완 필요">보완 요청</span>
										</c:when>
										<c:when test="${doc.status == 'Resubmit'}">
											<span class="status-badge status-resubmit"
												title="보완 후 재제출 요청">재제출 요청</span>
										</c:when>
									</c:choose></td>
								<td>${doc.full_name}</td>
