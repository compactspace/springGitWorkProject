...
<body>
		...
		<div id="carousel">
			<form action="${pageContext.request.contextPath}/users/order" id="forminfo">
				...
				<div class="order-actions"
					...
					<a href="productlist.do" class="continue-shopping"
  ...
  function extractImgUrl(imgStr) {
	...
	/*     html += '<input type="text" name="receipt_product_cod" value="' + item.productCod + '" readonly><br>';	   */
/* 	    html += 'item.productName'; */
	    ...
	    html += '<td class="col5"><input type="checkbox" class="finalladd"></td>';
  ...
  document.querySelector(".allchoicebtn").addEventListener("click", () => {
    document.querySelectorAll(".finalladd").forEach(cb => cb.checked = true);
  });

  document.querySelector(".allclearbtn").addEventListener("click", () => {
    document.querySelectorAll(".finalladd").forEach(cb => cb.checked = false);
  });
