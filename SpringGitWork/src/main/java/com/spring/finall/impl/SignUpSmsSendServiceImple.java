...
@Service
public class SignUpSmsSendServiceImple implements SignUpSmsSendService {
	...
	@Autowired
	private SignUpSmsSendServiceRedisDAO signUpSmsSendServiceRedisDAO;
	
	

	@Override
	public SignUpSmsSendResult requestSmsCode(String sessionId, String phone, HttpSession session, Model model) {
		
	   
		String cooldownKey = "sms:cooldown:" + sessionId + ":" + phone;
		String attemptKey = "sms:signup-attempt:" + sessionId + ":" + phone;

		long cooldownStartTime = 0;
		long cooldownDuration = 0;

		Long count = signUpSmsSendServiceRedisDAO.incrementAttempt(attemptKey);
		...
		if (count != null && count > 5) {
			return SignUpSmsSendResult.tooManyRequests();
		}

		// 2) 쿨다운 제한 (3분)
		if (Boolean.TRUE.equals(redisTemplate.hasKey(cooldownKey))) {
			return SignUpSmsSendResult.tooSoon();
		}
		...
		// 6) Redis에 발송 로그 저장
		String logKey = "sms:signup-log:" + sessionId + ":" + phone + ":" + now;
		signUpSmsSendServiceRedisDAO.setLog(logKey);
...
//		}
		...
		// 8) 성공 결과 반환
		return SignUpSmsSendResult.success(token, cooldownStartTime, cooldownDuration);
	}
	...
	@Override
	public SignUpSmsSendResult verifySmsCode(String sessionId,HttpSession session, String inputCode, String token) {
		...
		Long savedTime = (Long) session.getAttribute("authTokenTime");
		 ...
		 String verifiedKey = "signup:verified:session:" + sessionId;
			signUpSmsSendServiceRedisDAO.incrementAttempt(verifiedKey);
			redisTemplate.expire(verifiedKey, 5, TimeUnit.MINUTES); // 버전 호환
	
		
		
		if (savedCode == null || savedToken == null || savedTime == null) {
			return SignUpSmsSendResult.expired();
		}

		if (!savedToken.equals(token)) {
			return SignUpSmsSendResult.invalidTOKEN();
		}
		...
		if (!String.valueOf(savedCode).equals(inputCode)) {
			return SignUpSmsSendResult.invalidCODE();
		}
		...
		signUpSmsSendServiceRedisDAO.setVerified(sessionId);
		Long ttl = 	signUpSmsSendServiceRedisDAO.getVerifiedTTL(sessionId);
