		...
		try {
			...
			if (alreadyRefunded) {
				...
				String message = "해당 결제건" + String.valueOf(refundDTO.getPaymentId()) + " 에 대하여 이미 환불 신청이 되어있습니다.";
				  ...
				  throw new RequestRefundException(
			                "해당 결제건 " + refundDTO.getPaymentId() + " 에 이미 환불 신청이 되어있습니다.", 40001
			            );
			}
	        ...
	        throw new RequestRefundException(
	            "이미 환불 요청이 접수된 결제건입니다: " + refundDTO.getPaymentId(), 40001
	        );
	    } catch (Exception e) {
	        // 기타 예외
	        throw new RequestRefundException(
	            "환불 신청 중 에러가 발생했습니다: " + refundDTO.getPaymentId(), 50000
	        );
	    }
